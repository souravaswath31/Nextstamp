...
export default async function ItineraryDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid gap-4 border border-line p-4 sm:grid-cols-2">
        <div>
      ...
      <section className="border border-line p-4">
        ...
        <p className="mt-1 font-display text-2xl text-coralDark">
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {(() => {
            ...
            return (
              <>
                <BudgetLine label="Lodging" amount={breakdown.lodging} />
                <BudgetLine label="Food" amount={breakdown.food} />
                <BudgetLine label="Local transport" amount={breakdown.transport} />
                <BudgetLine label="Activities" amount={breakdown.activities} />
              </>
            );
          })()}
        </div>
      ...
      <section>
        ...
        <div className="mt-3 space-y-3">
          {countries.map((country, i) => (
            <div key={country} className="border border-line p-4">
      ...
      <section>
        ...
        <div className="mt-4 space-y-0">
          {itinerary.days.map((day, idx) => (
            <div
              key={day.id}
              className={`py-4 ${idx > 0 ? "ticket-divider" : ""}`}
            >
              <div className="flex items-baseline gap-3">
                <span className="font-stamp text-sm text-stamp">
                  Day {day.dayNumber}
                </span>
              ...
              <div className="mt-2 flex flex-wrap gap-x-6 gap-y-1 font-body text-xs text-ink/50">
                {day.driveTime && <span>Drive: {day.driveTime}</span>}
                {day.lodgingSuggestion && <span>Stay: {day.lodgingSuggestion}</span>}
                {day.coffeeWifiSpot && <span>Remote-work stop: {day.coffeeWifiSpot}</span>}
              </div>
            </div>
          ))}
        </div>
        ...
        <section>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {itinerary.notes.map((note) => (
              <div key={note.id} className="border border-line border-t-[3px] border-t-catSplurge bg-paper p-4">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {relatedStates.map((s) => (
              <Link
                key={s.id}
                href={`/state/${s.slug}`}
                className="group block border border-line border-t-[3px] border-t-catSplurge bg-paper p-4 transition-colors hover:border-ink"
              >
      ...
      <section>
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          {generatePackingList(itinerary.category.split(",").filter(Boolean)).map((section) => (
            <div key={section.title} className="border border-line p-4">
              <p className="font-stamp text-[11px] uppercase tracking-wide text-stamp">{section.title}</p>
              <ul className="mt-2 list-inside list-disc font-body text-sm text-ink/80">
      ...
      <form action={createTrip} className="pt-2">
        <button
          type="submit"
          className="rounded-full border border-coral bg-coral px-6 py-3 font-body text-sm font-semibold text-white shadow-sm hover:bg-coralDark"
        >
